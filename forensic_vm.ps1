...
function Get-PythonExe {
    ...
    # installs without PATH entries: PEP 514 registration, then default folders
    foreach ($root in @('Registry::HKEY_CURRENT_USER\Software\Python\PythonCore',
                        'Registry::HKEY_LOCAL_MACHINE\Software\Python\PythonCore',
                        'Registry::HKEY_LOCAL_MACHINE\Software\WOW6432Node\Python\PythonCore')) {
        try {
            foreach ($k in @(Get-ChildItem -Path $root -ErrorAction Stop | Sort-Object PSChildName -Descending)) {
                $ip = Get-ItemProperty -Path (Join-Path $k.PSPath 'InstallPath') -ErrorAction SilentlyContinue
                if (-not $ip) { continue }
                $exe = $null
                if ($ip.PSObject.Properties['ExecutablePath']) { $exe = $ip.ExecutablePath }
                if (-not $exe -and $ip.PSObject.Properties['(default)'] -and $ip.'(default)') {
                    $exe = Join-Path $ip.'(default)' 'python.exe'
                }
                if ($exe -and (Test-Path -LiteralPath $exe)) {
                    [void]$cands.Add([pscustomobject]@{ Exe = $exe; Pre = @() })
                }
            }
        } catch { }
    }
    foreach ($pattern in @("$env:LOCALAPPDATA\Programs\Python\Python3*\python.exe",
        ...
        if (-not $env:LOCALAPPDATA -and $pattern -like '\*') { continue }
        foreach ($f in @(Get-ChildItem -Path $pattern -ErrorAction SilentlyContinue | Sort-Object FullName -Descending)) {
            [void]$cands.Add([pscustomobject]@{ Exe = $f.FullName; Pre = @() })
        }
    }
    foreach ($c in $cands) {
...
function Install-WingetTool([string]$Name, $Spec) {
    ...
    if ($code -eq 0 -or ($WINGET_PRESENT -contains $hex)) {
        Update-SessionPath
        if ($code -eq 0) { $res = New-Result 'ok' }
        else { Write-Log "already installed ($hex)" 8; $res = New-Result 'present' }
        if ($Name -eq 'python') {
            $script:PythonExe = $null
            $py = Get-PythonExe
            if ($py) { Write-Log "interpreter: $py" 8 }
            elseif ($PortablePython) {
                Write-Log '[!] winget reports Python installed, but no working interpreter was found; using the portable package' 8
                return (Install-PortablePython)
            }
            else {
                Write-Log '[!] winget reports Python installed, but no working interpreter was found.' 8
                Write-Log 'Check Settings > Apps > Installed apps for "Python", or re-run with -PortablePython.' 8
                return (New-Result 'installed, interpreter not found')
            }
        }
        return $res
    }
